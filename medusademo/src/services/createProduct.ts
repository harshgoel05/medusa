...
class SanityService extends BaseService {
  ...
  mapUpdateToMedusa = (updatedField: any) => {
  ...
  updateLocalizedStringArray = (currentValue: any, newValue: string) => {
  ...
  createLocalizedStringArray = (value: string) => {
    return [
      {
        _key: "en",
        value,
  ...
  transformMedusaOptions = (
    variant: ProductVariant,
    options: ProductOptionValue[]
  ) =>
    options.map((option) => ({
      _key: option.id,
      value: this.createLocalizedStringArray(option.value),
      option_id: option.id,
      variant_id: variant.id,
    }));

  transformMedusaPrices = (prices: MoneyAmount[]) =>
    prices.map((price) => ({
      _key: price.id,
    ...
    const sanityImages = await Promise.all(
      images.map(async (image) => {
        ...
        if (existingSanityImage) {
          ...
          console.error(`Image with id ${image.id} was not found in sanity`);
        }
      })
    );
  ...
  async createProductInSanity(product) {
    try {
      const medusaProduct = await this.productService_.retrieve(product.id, {
        relations: [
          "variants",
          "options",
          "tags",
      ...
      const sanityVariants = medusaProduct.variants
        ? await Promise.all(
            medusaProduct.variants.map(async (variant) => {
              ...
              const sanityVariant = await this.createProductVariantInSanity(
                variant
              );
              return {
      ...
      const sanityProductFields = {
        ...
        title: [
          {
            _key: "en",
            value: medusaProduct.title,
          },
        ],
        subtitle: [
          {
            _key: "en",
            value: medusaProduct.subtitle,
          },
        ],
        description: [
          {
            _key: "en",
            value: medusaProduct.description,
          },
        ],
        handle: medusaProduct.handle,
        type: medusaProduct.type?.value,
        tags: medusaProduct.tags?.map((tag) => tag.value) || [],
        options: medusaProduct.options?.map((option) => option.title) || [],
      ...
      if (medusaProduct.thumbnail) {
        ...
        if (sanityImage) {
          // sanityProductFields["thumbnail"] = createImageField(sanityImage);
        }
      }

      // if (medusaProduct.images?.length > 0) {
      //   const sanityImages = await this.getImagesInSanity(medusaProduct.images);
      //   if (sanityImages.length > 0) {
      //     sanityProductFields["images"] = sanityImages.map((image) => {
  ...
  async updateProductInSanity(product) {
    try {
      ...
      const updatedFields = {
        title: this.updateLocalizedStringArray(
          productEntry.title,
          medusaProduct.title
        ),
        subtitle: this.updateLocalizedStringArray(
          productEntry.subtitle,
          medusaProduct.subtitle
        ),
        description: this.updateLocalizedStringArray(
    ...
    try {
      ...
      const sanityVariantFields = {
        ...
        title: [
          {
            _key: "en",
            value: medusaVariant.title,
          },
        ],
        sku: medusaVariant.sku,
        inventory_quantity: medusaVariant.inventory_quantity,
        prices: this.transformMedusaPrices(medusaVariant.prices),
        options: this.transformMedusaOptions(
          medusaVariant,
  ...
  async createCollectionInSanity(collection) {
    try {
      const medusaCollection = await this.productCollectionService_.retrieve(
        collection.id
      );
  ...
  async updateCollectionInSanity(collection) {
    try {
      ...
      return this.sanityClient
        ...
        .set({
          name: this.updateLocalizedStringArray(
  ...
  async handleSanityProductUpdate(sanityUpdate: any) {
    ...
    if (product.title) update.title = this.mapUpdateToMedusa(product.title);
    if (product.subtitle)
      update.subtitle = this.mapUpdateToMedusa(product.subtitle);
    if (product.description)
      update.description = this.mapUpdateToMedusa(product.description);
